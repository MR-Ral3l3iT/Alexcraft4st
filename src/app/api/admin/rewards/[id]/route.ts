...
const REWARD_UPLOAD_PREFIX = "/uploads/rewards/";
...
function resolveRewardPath(imageUrl: string): string | null {
  if (!imageUrl.startsWith(REWARD_UPLOAD_PREFIX)) return null;
  const filename = path.basename(imageUrl);
  return path.join(process.cwd(), "public", "uploads", "rewards", filename);
}

async function removeRewardImageFile(imageUrl: string): Promise<void> {
  const absolutePath = resolveRewardPath(imageUrl);
...
export async function PATCH(request: NextRequest, context: RouteContext) {
  ...
  const limiter = rateLimit(`admin:rewards:patch:${ip}`, 30, 60_000);
  if (!limiter.allowed) {
    return NextResponse.json({ message: "Too many requests" }, { status: 429 });
  }

  const { id: rawId } = await context.params;
  const id = sanitizeText(rawId, 80);
  if (!id) {
  ...
  if (!current) {
    return NextResponse.json({ message: "ไม่พบข้อมูลของรางวัล" }, { status: 404 });
  }
  ...
  if (imageFile instanceof File && imageFile.size > 0) {
    try {
      ...
      imageReplaced = imageUrl !== current.imageUrl;
    } catch (error) {
  ...
  if (imageReplaced) {
    await removeRewardImageFile(current.imageUrl);
  }
...
export async function DELETE(request: NextRequest, context: RouteContext) {
  const ip = request.headers.get("x-forwarded-for") || "local";
  const limiter = rateLimit(`admin:rewards:delete:${ip}`, 30, 60_000);
  if (!limiter.allowed) {
    return NextResponse.json({ message: "Too many requests" }, { status: 429 });
  }
  ...
  await removeRewardImageFile(reward.imageUrl);
